...
describe Spree::Chimpy::Interface::List do
  ...
  before do
    ...
    list_api.stub(:members).with(Digest::MD5.hexdigest(email)).and_return(members_api)
  ...
  context "#subscribe" do
    it "subscribes" do
      expect(members_api).to receive(:upsert)
  ...
  context "#unsubscribe" do
    it "unsubscribes" do
      expect(members_api).to receive(:update).with(
    ...
    it "ignores exception Gibbon::MailChimpError" do
      expect(members_api).to receive(:update).and_raise Gibbon::MailChimpError
