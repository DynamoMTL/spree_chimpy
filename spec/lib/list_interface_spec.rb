...
describe Spree::Chimpy::Interface::List do
  ...
  describe "#subscribe" do
    ...
    it "handles Mailchimp ListInvalidImportError error" do
      expect(api.lists).to receive(:member_info)
      expect(api.lists).to receive(:subscribe).and_raise Mailchimp::ListInvalidImportError
      interface.subscribe("user@example.com")
    end

    it "handles Mailchimp Validation error" do
      expect(api.lists).to receive(:member_info)
      expect(api.lists).to receive(:subscribe).and_raise Mailchimp::ValidationError
      interface.subscribe("user@example.com")
    end
  end
