...
describe Spree::Chimpy::Interface::Orders do
  ...
  it "adds an order" do
    ...
    api.ecomm.should_receive(:order_add) { |h| h[:id].should == order.number }.and_return(true)
  ...
  it "removes an order" do
    Spree::Chimpy::Config.store_id = "super-store"
    api.ecomm.should_receive(:order_del).with('super-store', order.number).and_return(true)
